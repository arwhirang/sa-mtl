...
def _cumsum(x, length):
    lower_triangular_ones = tf.constant(np.tril(np.ones((length, length))), dtype=tf.float32)
    return tf.reshape( tf.matmul(lower_triangular_ones, tf.reshape(x, [length, 1])), [length])


def _backwards_cumsum(x, length):
    upper_triangular_ones = tf.constant(np.triu(np.ones((length, length))), dtype=tf.float32)
    return tf.reshape(tf.matmul(upper_triangular_ones, tf.reshape(x, [length, 1])), [length])


class RNN(object):
    def __init__(self, num_emb, emb_dim, hidden_dim, sequence_length, start_token, learning_rate=0.01, reward_gamma=0.9):
        ...
        self.x = tf.compat.v1.placeholder(tf.int32, shape=[
            self.sequence_length])  # sequence of indices of true data, not including start token
        ...
        gen_o = tf.TensorArray(dtype=tf.float32, size=self.sequence_length, dynamic_size=False, infer_shape=True)
        gen_x = tf.TensorArray(dtype=tf.int32, size=self.sequence_length, dynamic_size=False, infer_shape=True)
        samples = tf.TensorArray(dtype=tf.float32, size=self.sequence_length)
        ...
        d_gen_predictions = tf.TensorArray(dtype=tf.float32, size=self.sequence_length,
                                           dynamic_size=False, infer_shape=True)
        d_real_predictions = tf.TensorArray(dtype=tf.float32, size=self.sequence_length,
                                            dynamic_size=False, infer_shape=True)

        self.gen_x = self.gen_x.stack()
        emb_gen_x = tf.gather(self.d_embeddings, self.gen_x)
        ta_emb_gen_x = tf.TensorArray(dtype=tf.float32, size=self.sequence_length)
        ta_emb_gen_x = ta_emb_gen_x.unstack(emb_gen_x)

        emb_real_x = tf.gather(self.d_embeddings, self.x)
        ta_emb_real_x = tf.TensorArray(dtype=tf.float32, size=self.sequence_length)
        ...
        _, _, _, self.d_gen_predictions = tf.while_loop(
        ...
        _, _, _, self.d_real_predictions = tf.while_loop(
            cond=lambda i, _1, _2, _3: i < self.sequence_length,
            body=_d_recurrence,
        ...
        self.d_gen_loss = tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.d_gen_predictions, labels=tf.zeros([self.sequence_length])))
        self.d_real_loss = tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        ...
        self.reward_loss = tf.math.reduce_mean(normalized_rewards ** 2)
        self.g_loss = -tf.math.reduce_mean(tf.math.log(self.gen_o.stack()) * normalized_rewards)
        ...
        reward_opt = tf.compat.v1.train.AdamOptimizer(self.learning_rate)
    ...
    def train_g_step(self, session):
        outputs = session.run(
            ...
            feed_dict={self.h0: np.random.normal(size=self.hidden_dim), self.samples: np.random.random(self.sequence_length)})
        return outputs

    def train_d_gen_step(self, session):
        outputs = session.run([self.d_gen_updates, self.d_gen_loss],
                              feed_dict={self.h0: np.random.normal(size=self.hidden_dim),
                                         self.samples: np.random.random(self.sequence_length)})
        return outputs

    def train_d_real_step(self, session, x):
        outputs = session.run([self.d_real_updates, self.d_real_loss], feed_dict={self.x: x})
        return outputs

    def pretrain_step(self, session, x):
        outputs = session.run([self.pretrain_updates, self.pretrain_loss, self.g_predictions],
                              feed_dict={self.x: x, self.h0: np.random.normal(size=self.hidden_dim)})
        return outputs
    ...
    def create_output_unit(self, params, embeddings):
        ...
        def unit(h_t):
            logits = tf.reshape(self.b_out2 +
                                tf.matmul(embeddings,
                                          tf.math.tanh(self.b_out1 +
    ...
    def d_optimizer(self, lr):
        return tf.compat.v1.train.AdamOptimizer(lr)

    def g_optimizer(self, lr):
        return tf.compat.v1.train.AdamOptimizer(lr)
...
class GRU(RNN):
    ...
    def create_recurrent_unit(self, params):
        ...
        def unit(x_t, h_tm1):
            ...
            r = tf.math.sigmoid(tf.matmul(self.W_rx, x_t) + tf.matmul(self.U_rh, h_tm1))
            z = tf.math.sigmoid(tf.matmul(self.W_zx, x_t) + tf.matmul(self.U_zh, h_tm1))
            h_tilda = tf.math.tanh(tf.matmul(self.W_hx, x_t) + tf.matmul(self.U_hh, r * h_tm1))
